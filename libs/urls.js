export const getClickUrls = (inputType, convertedType, bbox, z, x, y) => [
  ...
  `https://map.fgislk.gov.ru/map/nspd/api/aeggis/v3/${inputType}/wms?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetFeatureInfo&FORMAT=image/png&TRANSPARENT=true&QUERY_LAYERS=${inputType}&LAYERS=${inputType}&INFO_FORMAT=application/json&FEATURE_COUNT=10&RANDOM=${Math.random()}&I=49&J=405&WIDTH=512&HEIGHT=512&CRS=EPSG:3857&STYLES=&${bbox}`
...
export const getTileUrls = (inputType, convertedType, bbox, z, x, y) => [
  ...
  `https://map.fgislk.gov.ru/map/nspd/api/aeggis/v2/${inputType}/wms?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&FORMAT=image/png&TRANSPARENT=true&LAYERS=${inputType}&STYLES=&RANDOM=${Math.random()}&WIDTH=512&HEIGHT=512&CRS=EPSG:3857&BBOX=${bbox}`,
